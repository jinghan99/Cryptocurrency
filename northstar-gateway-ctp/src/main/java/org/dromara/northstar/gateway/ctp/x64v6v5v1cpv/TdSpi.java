...
import org.dromara.northstar.gateway.ctp.x64v6v3v15v.CtpConstant;
...
public class TdSpi extends CThostFtdcTraderSpi {
	...
	private static final IDataSource EMPTY_DS = new EmptyDataSource();
	...
	private List<Order> orderBuilderCacheList = new LinkedList<>(); // 登录起始阶段缓存Order
	private List<Trade> tradeBuilderCacheList = new LinkedList<>(); // 登录起始阶段缓存Trade
	...
	public void connect() {
		...
		try {
			String tdHost = settings.getHost();
	...
	public String submitOrder(SubmitOrderReq submitOrderReq) {
		...
		cThostFtdcInputOrderField.setForceCloseReason(CtpConstant.forceCloseReasonMap.get(ForceCloseReasonEnum.FCR_Unknown));
	...
	public boolean cancelOrder(String originOrderId) {
		...
		if (StringUtils.isBlank(originOrderId)) {
			logger.error("{}订单ID为空,无法撤单", logInfo);
			return false;
		}
	...
	private void reqAuth() {
		...
		try {
			...
			logger.debug("CTP 认证信息： APPID:{}, AUTH_CODE:{}, BROKER_ID:{}, PRODUCT_INFO: {}, USER_ID:{}", 
					settings.getAppId(), settings.getAuthCode(), settings.getBrokerId(), settings.getAppId(), settings.getUserId());
			...
			if (pRspInfo != null) {
				if (pRspInfo.getErrorID() == 0) {
					...
					logger.debug("CTP 登陆信息： BROKER_ID:{}, USER:{}, PWD:{}", 
							settings.getAppId(), settings.getUserId(), settings.getPassword());
	...
	public void OnRspQryInvestorPosition(CThostFtdcInvestorPositionField pInvestorPosition, CThostFtdcRspInfoField pRspInfo, int nRequestID, boolean bIsLast) {
		try {
			...
			Contract contract = gatewayAdapter.mktCenter.getContract(ChannelType.CTP_SIM, symbol).contract();
	...
	public void OnRspQryInstrument(CThostFtdcInstrumentField pInstrument, CThostFtdcRspInfoField pRspInfo, int nRequestID, boolean bIsLast) {
		try {
			...
			LocalDate dueDate = LocalDate.parse(pInstrument.getExpireDate(), DateTimeConstant.D_FORMAT_INT_FORMATTER);
			
			CtpContract contract = CtpContract.builder()
					.dataSrc(EMPTY_DS)
					.gatewayId(ChannelType.CTP_SIM.toString())
					.channelType(ChannelType.CTP_SIM)
					.symbol(symbol)
					.name(pInstrument.getInstrumentName())
			...
			if (bIsLast) {
				...
				for (Order order : orderBuilderCacheList) {
					try {
						Contract c = gatewayAdapter.mktCenter.getContract(ChannelType.CTP_SIM, order.contract().symbol()).contract();
				...
				for (Trade trade : tradeBuilderCacheList) {
					try {
						Contract c = gatewayAdapter.mktCenter.getContract(ChannelType.CTP_SIM, trade.contract().symbol()).contract();
				...
				gatewayAdapter.mktCenter.loadContractGroup(ChannelType.CTP_SIM);
	...
	public void OnRtnOrder(CThostFtdcOrderField pOrder) {
		...
		try {
			...
			LocalDate orderDate = LocalDate.parse(pOrder.getInsertDate(), DateTimeConstant.D_FORMAT_INT_FORMATTER);
			LocalTime orderTime = LocalTime.parse(pOrder.getInsertTime(), DateTimeConstant.T_FORMAT_FORMATTER);
			...
			if (instrumentQueried.get()) {
				Contract c = gatewayAdapter.mktCenter.getContract(ChannelType.CTP_SIM, symbol).contract();
				order = order.toBuilder().contract(c).build();
				gatewayAdapter.getEventEngine().emitEvent(NorthstarEventType.ORDER, order);
			} else {
	...
	public void OnRtnTrade(CThostFtdcTradeField pTrade) {
		...
		try {
			...
			LocalDate tradeDate = LocalDate.parse(pTrade.getTradeDate(), DateTimeConstant.D_FORMAT_INT_FORMATTER);
			LocalTime tradeTime = LocalTime.parse(pTrade.getTradeTime(), DateTimeConstant.T_FORMAT_FORMATTER);
