...
public class CtpSimGatewayAdapter extends GatewayAbstract implements MarketGateway, TradeGateway {
		...
		try {
			switch(Platform.current()) {
				case WINDOWS -> {
					...
					CommonUtils.copyURLToFileForTmp(tempLibPath, CtpSimGatewayAdapter.class.getResource("/assembly/libiconv.dll"));
					CommonUtils.copyURLToFileForTmp(tempLibPath, CtpSimGatewayAdapter.class.getResource("/assembly/jctpv6v5v1cpx64api/thostmduserapi_se.dll"));
					CommonUtils.copyURLToFileForTmp(tempLibPath, CtpSimGatewayAdapter.class.getResource("/assembly/jctpv6v5v1cpx64api/thosttraderapi_se.dll"));
					CommonUtils.copyURLToFileForTmp(tempLibPath, CtpSimGatewayAdapter.class.getResource("/assembly/jctpv6v5v1cpx64api/jctpv6v5v1cpx64api.dll"));
				...
				case LINUX -> {
					...
					CommonUtils.copyURLToFileForTmp(tempLibPath, CtpSimGatewayAdapter.class.getResource("/assembly/jctpv6v5v1cpx64api/libthostmduserapi_se.so"));
					CommonUtils.copyURLToFileForTmp(tempLibPath, CtpSimGatewayAdapter.class.getResource("/assembly/jctpv6v5v1cpx64api/libthosttraderapi_se.so"));
					CommonUtils.copyURLToFileForTmp(tempLibPath, CtpSimGatewayAdapter.class.getResource("/assembly/jctpv6v5v1cpx64api/libjctpv6v5v1cpx64api.so"));
	...
	@Override
	public boolean unsubscribe(Contract contractField) {
		if (gatewayDescription.getGatewayUsage() == GatewayUsage.MARKET_DATA) {
			if (mdSpi == null) {
				logger.error("{} 行情接口尚未初始化或已断开", logInfo);
				return false;
			}
			return mdSpi.unsubscribe(contractField.symbol());
		}
